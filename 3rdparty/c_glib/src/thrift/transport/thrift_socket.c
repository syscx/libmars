...
enum _ThriftSocketProperties {
    PROP_0, PROP_THRIFT_SOCKET_HOSTNAME, PROP_THRIFT_SOCKET_PORT
};
...
gboolean thrift_socket_open(ThriftTransport *transport, GError **error) {
    ...
    l.l_onoff = 1;
    l.l_linger = 0;
    if (setsockopt(tsocket->sd, SOL_SOCKET, SO_LINGER, &l, sizeof(l)) == -1) {
    ...
    if (connect(tsocket->sd, (struct sockaddr *) &pin, sizeof(pin)) == -1) {
        g_set_error(error, THRIFT_TRANSPORT_ERROR,
                ...
                "failed to connect to host %s:%d - %s", tsocket->hostname,
                tsocket->port, strerror(errno));
        return FALSE;
    }
    int flags = 0;
    flags = fcntl(tsocket->sd, F_GETFL, 0);
    if (-1 == fcntl(tsocket->sd, F_SETFL, flags | O_NONBLOCK)) {
        printf("failed to set nonblock - %s", strerror(errno));
        return -1;
    }
    return TRUE;
}
...
gint32 thrift_socket_read(ThriftTransport *transport, gpointer buf, guint32 len,
        GError **error) {
//    gint ret = 0;
//    guint got = 0;
...
//        got += ret;
//    }
    if (directRecv(socket->sd, buf, len, 3) == -1) {
        g_set_error(error, THRIFT_TRANSPORT_ERROR,
                THRIFT_TRANSPORT_ERROR_RECEIVE, "failed to read %d bytes - %s",
                len, strerror(errno));
        return -1;
    } else {
        return len;
...
//        if (ret < 0) {
//            g_set_error(error, THRIFT_TRANSPORT_ERROR,
//                    THRIFT_TRANSPORT_ERROR_SEND, "failed to send %d bytes - %s",
//                    len, strerror(errno));
//            return FALSE;
//        }
//        sent += ret;
//    }
    if (directSend(socket->sd, buf, len, 3) == -1) {
...
static void thrift_socket_init(ThriftSocket *socket) {
    socket->sd = 0;
}

/* destructor */
static void thrift_socket_finalize(GObject *object) {
...
/* property accessor */
void thrift_socket_get_property(GObject *object, guint property_id,
...
/* property mutator */
void thrift_socket_set_property(GObject *object, guint property_id,
...
static void thrift_socket_class_init(ThriftSocketClass *cls) {
    ...
    param_spec = g_param_spec_string("hostname", "hostname (construct)",
            "Set the hostname of the remote host", "localhost", /* default value */
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE);
    g_object_class_install_property(gobject_class, PROP_THRIFT_SOCKET_HOSTNAME,
            param_spec);

    param_spec = g_param_spec_uint("port", "port (construct)",
            "Set the port of the remote host", 0, /* min */
            65534, /* max */
