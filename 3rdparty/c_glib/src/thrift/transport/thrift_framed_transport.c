...
G_DEFINE_TYPE(ThriftFramedTransport, thrift_framed_transport,
...
gboolean thrift_framed_transport_read_frame(ThriftTransport *transport,
    ...
    /* read the size */
    THRIFT_TRANSPORT_GET_CLASS (t->transport)->read(
            t->transport, (guint32 *) &sz, sizeof(sz), error);
    if (error && *error) {
        printf("GError:%s\n", (*error)->message);
        return FALSE;
    }
...
gboolean thrift_framed_transport_write_slow(ThriftTransport *transport,
        gpointer buf, guint32 len, GError **error) {
    THRIFT_UNUSED_VAR(error);
    ThriftFramedTransport *t = THRIFT_FRAMED_TRANSPORT (transport);
...
gboolean thrift_framed_transport_write(ThriftTransport *transport,
        const gpointer buf, const guint32 len, GError **error) {
    ThriftFramedTransport *t = THRIFT_FRAMED_TRANSPORT (transport);
...
static void thrift_framed_transport_init(ThriftFramedTransport *transport) {
...
/* destructor */
static void thrift_framed_transport_finalize(GObject *object) {
...
/* property accessor */
void thrift_framed_transport_get_property(GObject *object, guint property_id,
...
/* property mutator */
void thrift_framed_transport_set_property(GObject *object, guint property_id,
...
static void thrift_framed_transport_class_init(ThriftFramedTransportClass *cls) {
    ...
    param_spec = g_param_spec_object("transport", "transport (construct)",
            "Thrift transport", THRIFT_TYPE_TRANSPORT,
            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    g_object_class_install_property(gobject_class,
            PROP_THRIFT_FRAMED_TRANSPORT_TRANSPORT, param_spec);

    param_spec = g_param_spec_uint("r_buf_size", "read buffer size (construct)",
            "Set the read buffer size", 0, /* min */
            1048576, /* max, 1024*1024 */
            512, /* default value */
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE);
    g_object_class_install_property(gobject_class,
            PROP_THRIFT_FRAMED_TRANSPORT_READ_BUFFER_SIZE, param_spec);

    param_spec = g_param_spec_uint("w_buf_size",
            "write buffer size (construct)", "Set the write buffer size", 0, /* min */
            1048576, /* max, 1024*1024 */
            512, /* default value */
            G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE);
    g_object_class_install_property(gobject_class,
            PROP_THRIFT_FRAMED_TRANSPORT_WRITE_BUFFER_SIZE, param_spec);
